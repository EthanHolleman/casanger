...
def blast_row_to_feature(row, color='#ffd700'):
    ...
    return GraphicFeature(
        start=start, end=end, strand=strand, color=color,
                   label=row_dict['read_type']
    )
...
def prepare_trace_dict(row):
    ...
    def slice_trace(trace):
        ...
        for each_channel in trace:
            trace[each_channel] = trace[each_channel][min(alignment_read_span):max(alignment_read_span)]
        return trace
...
def big_plot(reads):
    ...
    fig, subs = plt.subplots(
        len(reads)+1, 1, figsize=(12, 3), sharex=True, gridspec_kw={"height_ratios": [4, 1]}
    )
    # plot traces
    ...
    def plot_trace(trace_dict, axis):
        for i, each_channel in enumerate(trace):
            axis.plot(
                trace[each_channel], color=COLORS[i]
            )
        axis.axis('off')
        return axis
...
def main():
    ...
    template = ''
    alignment_feats = [blast_row_to_feature(row) for i, row in blast.iterrows()]
